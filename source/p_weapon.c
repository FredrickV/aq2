	  ...
	  switch (ent->client->curr_weap)
		...
		if(hc_single->value)
		{
			...
			if(ent->client->cannon_rds == 1)
				count = 1;
			else if ((ent->client->pers.inventory[ent->client->ammo_index]) == 1)
				count = 1;
			else
...
Weapon_GrenadeLauncher (edict_t * ent)
{
  ...
  Weapon_Generic (ent, 5, 16, 59, 64, 0, 0, pause_frames, fire_frames,
...
Weapon_RocketLauncher (edict_t * ent)
{
  ...
  Weapon_Generic (ent, 4, 12, 50, 54, 0, 0, pause_frames, fire_frames,
...
Weapon_Blaster (edict_t * ent)
{
  ...
  Weapon_Generic (ent, 4, 8, 52, 55, 0, 0, pause_frames, fire_frames,
		  Weapon_Blaster_Fire);
}
...
Weapon_HyperBlaster_Fire (edict_t * ent)
{
  ...
  if (ent->client->ps.gunframe == 12)
    {
      gi.sound (ent, CHAN_AUTO, gi.soundindex ("weapons/hyprbd1a.wav"), 1,
		ATTN_NORM, 0);
      ent->client->weapon_sound = 0;
    }
...
Weapon_HyperBlaster (edict_t * ent)
{
  ...
  Weapon_Generic (ent, 5, 20, 49, 53, 0, 0, pause_frames, fire_frames,
...
Weapon_Machinegun (edict_t * ent)
{
  ...
  Weapon_Generic (ent, 3, 5, 45, 49, 0, 0, pause_frames, fire_frames,
		  Machinegun_Fire);
}
...
Chaingun_Fire (edict_t * ent)
{
  ...
  if (ent->client->ps.gunframe == 22)
    {
      ent->client->weapon_sound = 0;
      gi.sound (ent, CHAN_AUTO, gi.soundindex ("weapons/chngnd1a.wav"), 1,
		ATTN_IDLE, 0);
    }
  else
    {
...
Weapon_Chaingun (edict_t * ent)
{
  ...
  Weapon_Generic (ent, 4, 31, 61, 64, 0, 0, pause_frames, fire_frames,
		  Chaingun_Fire);
}
...
Weapon_Shotgun (edict_t * ent)
{
  ...
  Weapon_Generic (ent, 7, 18, 36, 39, 0, 0, pause_frames, fire_frames,
		  weapon_shotgun_fire);
}
...
Weapon_SuperShotgun (edict_t * ent)
{
  ...
  Weapon_Generic (ent, 6, 17, 57, 61, 0, 0, pause_frames, fire_frames,
...
Weapon_Railgun (edict_t * ent)
{
  ...
  Weapon_Generic (ent, 3, 18, 56, 61, 0, 0, pause_frames, fire_frames,
		  weapon_railgun_fire);
}
...
void Weapon_BFG (edict_t * ent)
{
  ...
  Weapon_Generic (ent, 8, 32, 55, 58, 0, 0, pause_frames, fire_frames,
		  weapon_bfg_fire);
}
...
void Pistol_Fire (edict_t * ent)
{
	...
	if (!teamplay->value || team_round_going || stats_afterround->value) {
		...
		ent->client->resp.stats_shots[MOD_MK23] += 1;	// TNG Stats, +1 hit
	}
	ent->client->mk23_rds--;
	ent->client->dual_rds--;
...
void MP5_Fire (edict_t * ent)
{
	...
	if (!teamplay->value || team_round_going || stats_afterround->value) {
		ent->client->resp.stats_shots_t += 1;	// TNG Stats, +1 hit
		ent->client->resp.stats_shots[MOD_MP5] += 1;	// TNG Stats, +1 hit
	}
	ent->client->mp5_rds--;
...
void M4_Fire (edict_t * ent)
{
	...
	if (!teamplay->value || team_round_going || stats_afterround->value) {
		ent->client->resp.stats_shots_t += 1;	// TNG Stats, +1 hit
		ent->client->resp.stats_shots[MOD_M4] += 1;	// TNG Stats, +1 hit
	}
	ent->client->m4_rds--;
...
void M3_Fire (edict_t * ent)
{
	...
	if (!teamplay->value || team_round_going || stats_afterround->value) {
		ent->client->resp.stats_shots_t += 1;	// TNG Stats, +1 hit
		ent->client->resp.stats_shots[MOD_M3] += 1;	// TNG Stats, +1 hit
	}
...
void HC_Fire (edict_t * ent)
{
	...
	if (ent->client->resp.hc_mode)
	{
		...
		if (!teamplay->value || team_round_going || stats_afterround->value) {
			ent->client->resp.stats_shots_t += 1;	// TNG Stats, +1 hit
			ent->client->resp.stats_shots[MOD_HC] += 1;	// TNG Stats, +1 hit
		}
		if (llsound->value == 0)
		{
...
void Sniper_Fire (edict_t * ent)
{
	...
	if (!teamplay->value || team_round_going || stats_afterround->value) {
		ent->client->resp.stats_shots_t += 1;	// TNG Stats, +1 hit
		ent->client->resp.stats_shots[MOD_SNIPER] += 1;	// TNG Stats, +1 hit
	}
	ent->client->sniper_rds--;
...
void Dual_Fire (edict_t * ent)
{
	...
	if (ent->client->ps.gunframe == 8)
	{
		...
		if (ent->client->dual_rds > 1)
		{
			...
			if (!teamplay->value || team_round_going || stats_afterround->value) {
				ent->client->resp.stats_shots_t += 1;	// TNG Stats, +1 hit
				ent->client->resp.stats_shots[MOD_DUAL] += 1;	// TNG Stats, +1 hit
			}
...
int Knife_Fire (edict_t * ent)
{
	...
	if (ent->client->resp.knife_mode == 0)
	{
		...
		if (!teamplay->value || team_round_going || stats_afterround->value) {
			ent->client->resp.stats_shots_t += 1;	// TNG Stats, +1 hit
			ent->client->resp.stats_shots[MOD_KNIFE] += 1;	// TNG Stats, +1 hit
		}
	...
	else
	{
		...
		if (INV_AMMO(ent, KNIFE_NUM) <= 0)
		{
			...
			if (!teamplay->value || team_round_going || stats_afterround->value) {
				ent->client->resp.stats_shots_t += 1;	// TNG Stats, +1 hit
				ent->client->resp.stats_shots[MOD_KNIFE_THROWN] += 1;	// TNG Stats, +1 hit
			}
			return 0;
		}
		else
